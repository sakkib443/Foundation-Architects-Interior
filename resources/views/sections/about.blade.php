@php
    $about = $settings->get('homepage.about', []);
    ...
    $aboutImageMain = $about['image_main'] ?? 'images/portfolio/project-2.jpg';
    $aboutImageSecondary = $about['image_secondary'] ?? 'images/portfolio/project-1.jpg';
    $aboutBadge = $about['badge'] ?? '100% Satisfaction';
    $aboutBadge2 = $about['badge2'] ?? ['value' => '10+', 'label' => 'Years Experience'];
@endphp
    ...
    <div class="mx-auto grid max-w-7xl items-center gap-14 px-4 sm:px-6 lg:grid-cols-2 lg:gap-16 lg:px-8">
        ...
        <div class="relative">
            ...
            <div class="relative overflow-hidden rounded-3xl shadow-2xl shadow-brand-900/10 ring-1 ring-black/5">
                <img src="{{ asset($aboutImageMain) }}" alt="Interior design project"
                ...
                <div class="absolute right-4 top-4 flex items-center gap-2 rounded-full bg-white/95 px-4 py-2 shadow-lg backdrop-blur">
                    ...
                    <span class="text-sm font-semibold text-brand-900">{{ $aboutBadge }}</span>
                </div>
            </div>
            ...
            <div class="absolute -bottom-8 left-5 hidden w-40 overflow-hidden rounded-2xl border-4 border-white shadow-xl sm:left-8 md:block">
                <img src="{{ asset($aboutImageSecondary) }}" alt="Interior detail"
            ...
            <div class="absolute -bottom-6 right-5 rounded-2xl bg-gradient-to-br from-brand-500 to-brand-700 px-6 py-4 text-center text-white shadow-xl sm:right-8">
                <p class="text-2xl font-bold leading-none">{{ $aboutBadge2['value'] ?? '' }}</p>
                <p class="mt-1 text-[11px] font-medium text-white/90">{{ $aboutBadge2['label'] ?? '' }}</p>
            </div>
        </div>
        ...
        <div>
            ...
            <h2 class="mt-5 font-display text-4xl font-bold leading-tight tracking-tight text-brand-900 sm:text-5xl">
                {{ $about['headline'] ?? 'Elegant, Functional & Timeless Interiors' }}
            </h2>

            <p class="mt-5 text-base leading-relaxed text-stone-600">
                {{ $about['body'] ?? 'Foundation Architects & Interior is a trusted name in the interior sector of Bangladesh — working with faith and honesty. From concept to handover, we design and build beautiful, functional spaces for homes, offices, and commercial projects — tailored to your taste, lifestyle, and budget.' }}
            </p>
                    ...
                    <div class="rounded-xl border border-brand-100 bg-white p-4 shadow-sm transition duration-300 hover:border-brand-300 hover:shadow-md hover:shadow-brand-500/5">
                        <div class="flex items-start gap-3">
                            ...
                            <div>
                                ...
                                <p class="mt-1 text-xs leading-relaxed text-stone-500">{{ $feature['description'] ?? '' }}</p>
                            </div>
                        </div>
                    </div>
                @endforeach
