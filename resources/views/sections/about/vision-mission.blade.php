@php
    $pillars = $settings->get('about.vision_mission', []);
    $pillars = (is_array($pillars) && ! empty($pillars)) ? $pillars : [
        [
            'tag'         => 'Our Vision',
...
<section class="bg-brand-50 py-20 sm:py-28">
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        ...
        <div class="mx-auto flex max-w-2xl flex-col items-center text-center">
            <p class="section-eyebrow text-brand-600">What Drives Us</p>
            ...
            @foreach ($pillars as $p)
                <div class="group relative overflow-hidden rounded-3xl border border-brand-100 bg-white p-8 shadow-sm transition duration-300 hover:-translate-y-1 hover:shadow-xl hover:shadow-brand-500/10 sm:p-10">
                    ...
                    <div class="relative">
                        ...
                        <p class="mt-4 leading-relaxed text-stone-600">{{ $p['description'] ?? '' }}</p>
                    </div>
                </div>
            @endforeach
        </div>
